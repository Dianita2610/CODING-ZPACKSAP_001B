...
*SELECTION-SCREEN BEGIN OF BLOCK b01 WITH FRAME TITLE text-001.
  PARAMETERS: lv_bukrs  TYPE bukrs        OBLIGATORY.    " Sociedad
  PARAMETERS: p_coste   TYPE mseg-kostl   OBLIGATORY.    " centro de  coste
  PARAMETERS: p_fechad  TYPE mkpf-bldat   OBLIGATORY.    " fecha de documento
...
* END. 08-07-2026 - ATC - ATC-01
        ...
        IF gt_alv[] IS NOT INITIAL.
          PERFORM mostrar_alv.
        ELSE.
          MESSAGE 'No existen datos para los paremetros seleccionados' TYPE 'S'.
        ENDIF.
      ENDIF.
    ENDIF.
  ENDIF.
...
*----------------------------------------------------------------------*
FORM seleccion_datos.
  "Con la fecha ingresada ir a la tabla MKPF y rescatar todos los MBLNR asociados. Luego ir a la tabla MSEG, ingresar estos datos y además el centro de costo KOSTL.
* BEGIN. 08-07-2026 - ATC - ATC-03
* OLD CODE
*  SELECT  mblnr INTO CORRESPONDING FIELDS OF TABLE gt_mkpf
*  FROM mkpf
*  WHERE bldat EQ p_fechad.
*
* NEW CODE
  SELECT mblnr
 INTO CORRESPONDING FIELDS OF TABLE gt_mkpf
  FROM mkpf
  WHERE bldat EQ p_fechad ORDER BY PRIMARY KEY.
...
* NEW CODE
 ...
 INTO CORRESPONDING FIELDS OF TABLE gt_alv
    ...
    WHERE mblnr EQ gt_mkpf-mblnr
      AND bwart IN ('201', '202' )
      AND kostl EQ p_coste
      AND zznfolio EQ '0000000000' ORDER BY PRIMARY KEY. 

* END. 08-07-2026 - ATC - ATC-03" que no tenga numero de folio asociado.

    LOOP AT gt_alv INTO gs_alv WHERE bwart EQ '202'.

      DELETE gt_alv WHERE mblnr = gs_alv-smbln AND " DOC MATERIAL
                          mjahr = gs_alv-sjahr AND " EJERCICIO DOC MATERIAL
                          zeile = gs_alv-smblp. " POSICION

***************************************
* HCD 05102015
***************************************
        DELETE gt_alv WHERE
              mjahr  = gs_alv-mjahr AND   " PERIODO
              matnr  = gs_alv-matnr  AND  " MATERIAL
              werks  = gs_alv-werks  AND  " CENTRO
*              lgorkt = gs_alv-lgorkt  AND " ALMACEN
              menge =  gs_alv-menge AND   " CANTIDAD
              meins =  gs_alv-meins AND   " UNIDAD
              zeile  =  gs_alv-zeile  AND " posicion
              bwart EQ '201'.
***************************************
***************************************
    ENDLOOP.

    DELETE gt_alv WHERE bwart EQ '202'.
...
*  l_layout-edit              = 'X'. "makes whole ALV table editable
...
*   Imprimir
    WHEN '&IMPRIMIR'.
      ...
      IF gs_alv IS INITIAL.
        MESSAGE 'Debe seleccionar al menos un registro para imprimir' TYPE 'E'.
      ELSEIF  lv_reg GT 24.
...
* END. 08-07-2026 - ATC - ATC-01
  ...
  LOOP AT gt_alv INTO gs_alv WHERE check EQ 'X'.
