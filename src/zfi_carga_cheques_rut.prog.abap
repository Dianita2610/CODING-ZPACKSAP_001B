...
* OLD CODE
*  SELECT bukrs belnr gjahr blart xref1_hd
*    INTO TABLE ti_bkpf
*    FROM bkpf
...
*      AND xref1_hd  EQ ti_entrada-vblnr
*      AND stblg     EQ space.
*
* NEW CODE
  SELECT bukrs belnr gjahr blart xref1_hd
...
*koart EQ 'D' ).
KOART EQ 'D' ). "ORDER BY PRIMARY KEY .            "MODIFY@DCR:27.05.2026
        SORT ti_bseg BY bukrs belnr gjahr.         "INSERT@DCR:27.05.2026
...
* END. 08-07-2026 - ATC - ATC-03
    ...
    LOOP AT ti_bkpf INTO wa_bkpf.
      READ TABLE ti_entrada INTO wa_entrada
      WITH KEY vblnr = wa_bkpf-xref1_hd.
      IF sy-subrc EQ 0.
        REFRESH: bdcdata, messtab.
        READ TABLE ti_bseg INTO wa_bseg WITH KEY bukrs = wa_bkpf-bukrs
                                                 belnr = wa_bkpf-belnr
                                                 gjahr = wa_bkpf-gjahr
                                                 BINARY SEARCH.
        IF sy-subrc EQ 0.
          ...
          IF wa_lfa1-ort01 IS INITIAL. "Si no tiene Población
            ...
            PERFORM bdc_field  USING 'PAYR-VBLNR'  wa_bkpf-belnr.
            PERFORM bdc_field  USING 'PAYR-ZBUKR'  wa_bkpf-bukrs.
            PERFORM bdc_field  USING 'PAYR-GJAHR'  wa_bkpf-gjahr.
            ...
            IF NOT wa_entrada-znme1 IS INITIAL.
              PERFORM bdc_field  USING 'PAYR-ZNME1'  wa_entrada-znme1.
            ENDIF.
          ...
          READ TABLE messtab WITH KEY msgid = 'FS'
                                      msgnr = 568.
          IF sy-subrc EQ 0 AND
          NOT wa_entrada-znme1 IS INITIAL.
            UPDATE payr SET znme1 = wa_entrada-znme1
              WHERE zbukr EQ wa_entrada-bukrs
                AND hbkid EQ wa_entrada-hbkid
                AND hktid EQ wa_entrada-hktid
                AND chect EQ wa_entrada-chect.
          ENDIF.
          ...
          LOOP AT messtab.
            ...
            CONCATENATE 'Doc' wa_bkpf-bukrs wa_bkpf-belnr
                       wa_bkpf-gjahr  wa_log-msgv1 INTO wa_log-msgv1
                        SEPARATED BY space.
        ...
        ELSE.
          ...
          wa_log-msgty = 'E'.
          CONCATENATE 'Doc' wa_bkpf-bukrs wa_bkpf-belnr
                      wa_bkpf-gjahr 'no existe' INTO wa_log-msgv1
                      SEPARATED BY space.
          APPEND wa_log TO ti_log.
        ENDIF.
      ...
      ELSE.
        ...
        wa_log-msgno = 398.
        wa_log-msgty = 'E'.
        CONCATENATE 'Doc' wa_bkpf-bukrs wa_bkpf-belnr wa_bkpf-gjahr
                    '- No existen datos de cheque'
