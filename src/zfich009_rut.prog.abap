...
FORM bajar_archivo .
  ...
  ELSE.
    ...
    IF sy-subrc EQ 0.
      DO.
        ...
        IF sy-subrc EQ 0.
          CLEAR ti_entrada.
          SPLIT cadena AT tab INTO ti_entrada-zbukr
                                   ti_entrada-hbkid
                                   ti_entrada-gjahr
                                   ti_entrada-chect_cad
                                   ti_entrada-chect_cob
                                   ti_entrada-lifnr
                                   ti_entrada-vblnr_sap
                                   ti_entrada-vblnr_nosap.

          CONDENSE ti_entrada-lifnr.

          CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
            EXPORTING
              input  = ti_entrada-lifnr
            IMPORTING
              output = ti_entrada-lifnr.
  ...
  SORT ti_entrada BY zbukr hbkid chect_cob gjahr.
...
*ReSQ: No Need Of Change Internal Table TI_ENTRADA Already Sorted
    READ TABLE ti_entrada WITH KEY zbukr     = <fs>-zbukr
                                   hbkid     = <fs>-hbkid
                                   chect_cob = <fs>-chect
                                   gjahr     = <fs>-gjahr
                                   BINARY SEARCH.
    IF sy-subrc EQ 0.
      <fs>-vblnr = ti_entrada-vblnr_sap.
      IF p_test IS INITIAL.
        DELETE FROM payr WHERE zbukr = ti_entrada-zbukr
                           AND hbkid = ti_entrada-hbkid
                           AND chect = ti_entrada-chect_cad
                           AND gjahr = ti_entrada-gjahr.
      ENDIF.
    ENDIF.
  ENDLOOP.

  IF p_test IS INITIAL.
    MODIFY payr FROM TABLE ti_payr.
  ENDIF.
ENDFORM.                    " PROCESAR
*&---------------------------------------------------------------------*
*&      Form  MOSTRAR_ALV
