...
* NEW CODE
  SELECT *
  UP TO 1 ROWS  FROM ESSR WHERE LBLNI EQ LES_LBLNI ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT *
    UP TO 1 ROWS  FROM EKKO WHERE EBELN EQ ESSR-EBELN ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT *
    UP TO 1 ROWS  FROM EKPO WHERE EBELN EQ ESSR-EBELN
                              AND   EBELP EQ ESSR-EBELP ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT *
 FROM EKET WHERE EBELN EQ EKPO-EBELN
                       AND   EBELP EQ EKPO-EBELP ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
 FROM EKPA WHERE EBELN = EKKO-EBELN AND PARVW = H_PARVW ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
      UP TO 1 ROWS  FROM LFA1 WHERE LIFNR = EKPA-LIFN2 ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT *
  UP TO 1 ROWS  FROM T024 WHERE EKGRP EQ EKKO-EKGRP ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT *
  UP TO 1 ROWS  FROM T024E WHERE EKORG EQ EKKO-EKORG ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT *
  UP TO 1 ROWS  FROM T001 WHERE BUKRS EQ EKKO-BUKRS ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT *
 FROM T166P INTO TABLE IT166P
           WHERE DRUVO EQ XDRUVO
           AND   BSTYP EQ 'Q'
           AND   BSART EQ EKKO-BSART
           AND   PSTYP EQ EKPO-PSTYP ORDER BY PRIMARY KEY.
...
* NEW CODE
  ...
  UP TO 1 ROWS  FROM  T001W
         WHERE  WERKS       = WAN_WERKS ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
      UP TO 1 ROWS  FROM TTXIT WHERE TDSPRAS  EQ EKKO-SPRAS
                                 AND   TDOBJECT EQ OBJECT
                                 AND   TDID     EQ ID ORDER BY PRIMARY KEY.
